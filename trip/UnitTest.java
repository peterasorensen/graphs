package trip;

/* You MAY add public @Test methods to this class.  You may also add
 * additional public classes containing "Testing" in their name. These
 * may not be part of your trip package per se (that is, it must be
 * possible to remove them and still have your package work). */

import org.junit.Test;
import ucb.junit.textui;
import static org.junit.Assert.*;

/** Unit tests for the trip package. */
public class UnitTest {

    /** Run all JUnit tests in the graph package. */
    public static void main(String[] ignored) {
        System.exit(textui.runClasses(trip.UnitTest.class));
    }

    @Test
    public void dummyTest() {
        Trip wow = new Trip();
        assertNotEquals(Direction.SN, Direction.EW);
        assertNotEquals(wow, new Trip());
    }

    @Test
    public void bigArse() {
        Trip didmy = new Trip();
        assertNotEquals(Direction.WE, new Trip());
        String meatgo;
        meatgo = "off";
        assertEquals(meatgo, "off");
        assertNotEquals(didmy, new Trip());
    }


}

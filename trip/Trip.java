...
class Trip {
    ...
    void readMap(String name) {
        ...
        n = 0;
        try {
            FileReader ye = new FileReader(name);
            Scanner inp = new Scanner(ye);
            while (inp.hasNext()) {
                n += 1;
                switch (inp.next()) {
                case "L":
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        String currTitle, currD, lastTitle;
        int lastVert, currVert;
        currTitle = currD = lastTitle = "";
        Iterator<Integer> iterSeg = segment.iterator();
        lastVert = iterSeg.next();
        currVert = iterSeg.next();
        Road lastRoad = _map.getLabel(lastVert, currVert);
        double lastDist = lastRoad.length();
        double currDist;
        String lastD = lastRoad.direction().fullName();
        String lastName = lastRoad.toString();
        while (iterSeg.hasNext()) {
            lastVert = currVert;
            currVert = iterSeg.next();
            Road currRoad = _map.getLabel(lastVert, currVert);
            currD = currRoad.direction().fullName();
            currDist = currRoad.length();
            currTitle = currRoad.toString();
            if (!(lastName.equals(currTitle) && lastD.equals(currD))) {
                double newDist = (double) Math.round(lastDist * 10) / 10;
                System.out.println(seq + ". Take " + lastName + " " + lastD
                        + " for " + newDist + " miles.");
                lastName = currTitle;
                lastD = currD;
                lastDist = currDist;
                ...
                lastDist = lastDist + currDist;
            }
            lastTitle = _map.getLabel(currVert).toString();
        }
        double locDist = (double) Math.round(lastDist * 10) / 10;
        System.out.println(seq + ". Take " + lastName + " " + lastD
                + " for " + locDist + " miles to " + lastTitle + ".");
        seq += 1;
        return seq;
    }
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road bumpy = new Road(name, dir, length);
        Road bumpier = new Road(name, dir.reverse(), length);
        _map.add(v0, v1, bumpy);
        _map.add(v1, v0, bumpier);
    }
